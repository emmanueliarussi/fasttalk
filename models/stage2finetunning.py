...
class CodeTalker(BaseModel):
    def __init__(self, args):
        ...
        self.pos_enc = PositionalEncoding(dim=args.feature_dim, dropout=self.dropout)
        ...
        self.style_frame_encoder = nn.TransformerEncoder(
                                                            nn.TransformerEncoderLayer(d_model=1024, nhead=4, batch_first=True, dropout=self.dropout),
                                                            num_layers=2
                                                        )
        # Optional: learn a fixed style vector for overfitting/memorization
        self.use_fixed_style = getattr(args, "use_fixed_style", False)
        if self.use_fixed_style:
            self.fixed_style = nn.Parameter(torch.zeros(1, style_dim))
            nn.init.normal_(self.fixed_style, mean=0.0, std=0.02)
            print("--> Using fixed style vector for training (no cropping).")

        # Residual learning (predict delta on top of frozen base model)
        self.use_residual = getattr(args, "use_residual", False)
        self.residual_scale = getattr(args, "residual_scale", 1.0)
    ...
    def init_residual_base(self):
        """Freeze a base copy of the current model for residual learning.
        ...
        self.base_audio_encoder = copy.deepcopy(self.audio_encoder).to(device)
        self.base_audio_feature_map = copy.deepcopy(self.audio_feature_map).to(device)
        self.base_blendshapes_map = copy.deepcopy(self.blendshapes_map).to(device)
        self.base_pos_enc = copy.deepcopy(self.pos_enc).to(device)
        self.base_transformer_decoder = copy.deepcopy(self.transformer_decoder).to(device)
        self.base_feat_map = copy.deepcopy(self.feat_map).to(device)

        for m in [
            self.base_audio_encoder,
            self.base_audio_feature_map,
            self.base_blendshapes_map,
            self.base_pos_enc,
            self.base_transformer_decoder,
            self.base_feat_map,
        ]:
        ...
        print("--> Residual base initialized and frozen.")
    ...
    def forward(self, padded_blendshapes, blendshapes_mask, padded_audios, audio_mask, criterion, target_style=None):
        ...
        # Deterministic style (no random cropping) for easier memorization
        if self.use_fixed_style:
            style_vec = F.normalize(self.fixed_style.expand(B, -1), dim=-1)
        else:
            style_vec = self._style_view(feat_q_gt.detach(), blendshapes_mask)  # [B, D]
        ...
        feat_out_pred = feat_out
        if self.use_residual and self._residual_ready:
            with torch.no_grad():
                emb_blendshapes_tgt_base = self.base_blendshapes_map(padded_blendshapes_shifted)
                emb_blendshapes_tgt_base = self.base_pos_enc(emb_blendshapes_tgt_base)

                feat_out_base = self.base_transformer_decoder(
                                                    tgt=emb_blendshapes_tgt_base,
                                                    memory=hidden_states,
                                                    style=style_vec,
                                                    tgt_mask=tgt_mask,
                                                    memory_mask=memory_mask,
                ...
                feat_out_base = self.base_feat_map(feat_out_base)

            feat_out_pred = feat_out_base.detach() + self.residual_scale * feat_out
        ...
        loss_reg     = criterion(feat_out_pred, encoded.detach())  # L2 loss between predicted and quantized ground truth features

        # Also make the deocoded blendshapes match the input blendshapes
        feat_out_q, _, _ = self.autoencoder.vq(feat_out_pred) # Quantize the embedding
        ...
        # Masked L2 loss to ignore padding frames
        mask = blendshapes_mask.unsqueeze(-1).to(blendshapes_out.dtype)  # [B, T, 1]
        diff = (blendshapes_out - padded_blendshapes) ** 2
        loss_blendshapes = (diff * mask).sum() / (mask.sum() * diff.shape[-1] + 1e-8)
      
        # --- add self-supervised style compactness ---------------------------------
        #loss_style = self.nt_xent_unsup(style_a, style_b)

        return loss_blendshapes + loss_reg,  [loss_blendshapes, loss_reg, loss_reg, loss_reg]
    ...
    def predict(self, audio, target_style=None):
        ...
        if self.use_fixed_style:
            style_vec = F.normalize(self.fixed_style.expand(audio.shape[0], -1), dim=-1)
        ...
        for i in range(frame_num):
            ...
            if self.use_residual and self._residual_ready:
                with torch.no_grad():
                    blendshapes_input_base = self.base_pos_enc(blendshapes_emb)
                    feat_out_base = self.base_transformer_decoder(
                                                    tgt=blendshapes_input_base,
                                                    memory=hidden_states,
                                                    tgt_mask=tgt_mask, 
                                                    memory_mask=memory_mask,
                                                    style   = style_vec,
        ...
        feat_out_q, _, _ = self.autoencoder.vq(feat_out_pred)
    ...
    def predict_no_quantizer(self, audio, target_style=None):
        ...
        for i in range(frame_num):
            ...
            # Quantized features to blendshapes
            if i == 0:
                blendshapes_out_q = self.autoencoder.decode(torch.cat([feat_out_pred, feat_out_pred], dim=1))
                blendshapes_out_q = blendshapes_out_q[:,0].unsqueeze(1)
            else:
                blendshapes_out_q = self.autoencoder.decode(feat_out_pred)
        ...
        blendshapes_out  = self.autoencoder.decode(feat_out_pred)
